...
def stft_generator(filename,noisyAudio,fs=8000):
    ...
    if noisyAudio == False:
        audio = np.nan_to_num(audio_data)
        stft_segments= librosa.stft(audio, n_fft=FrameSize, hop_length=Overlap, win_length=FFTSize,
...
counter = 0
for clear_audio in tqdm(clear_files[0:400]):
    clear_audio_fullname = folder + clear_audio
    mixture_fullname = clear_audio_fullname
    if counter == 0:
        ...
        with h5py.File('fullmixture_5k_2channels_sandbox.hdf5', 'w') as hf:
    ...
    elif counter > 0 and counter < 5000:
        # Concatenate operation
        if counter%200 == 0 and counter > 200:
            print(str(counter) + ' - ' + clear_audio_fullname)
            # ARRAY APPEND
            with h5py.File('fullmixture_5k_2channels_sandbox.hdf5', 'a') as hf:
